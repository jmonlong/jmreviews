...
A toolkit to manipulate alignments in the GAF format.
It provides ways to:

- index, sort, query alignments
- sort a graph (to allow for alignment indexing)
- realign sequences to the pangenome using WFA
- convert to/from stable(rGFA)/unstable(node IDs) coordinates.
- annotating alignments with phase information

It seems like it was developed with Minigraph-made pangenome because the (necessary) pangenome graph ordering "leverages the properties of rGFAs generated by Minigraph".
The ordering of the graph is based on top-level bubble order and then node order within that bubble.
So for example a node could be in top-level bubble 22, and within that be the second node, so coordinate BO=22, NO=2.
The node order within a bubble follows the node IDs.
In summary, if node IDs are topologically sorted it shouldn't be different than indexing by node IDs directly, and if not, large bubbles with nesting won't be indexed as efficiently as possible.
The alignments are sorted according to the coordinates of the first node (according to the doc).
An index can then quickly find the "coordinate" of a node and jump to the appropriate block in a bgzipped GAF file.

The realignment module seems very useful to improve alignments from GraphAligner.
They show an example of an SV/indel (51 bp deletion) where this affine gap-cost realignment clarifies pileup compared to the original GraphAligner version.

[gaftools](https://github.com/marschall-lab/gaftools) is a Python package installable with pip or bioconda.

Next is testing it, in particular to know: 

- Can we query any regions relative to any haplotype or just the reference path?
- If it sorts alignment using the first node only, does it extract all overlapping alignments or only those that start in the queried region?
...
# Association testing involving pangenomes

- Cochetel et al. Genome Biology 2023 [@cochetelSuperpangenomeNorthAmerican2023]
    - Assembly then PGGB
    - Deconstruct then vg construct another pangenome to align reads with vg map and variants genotyped with vg call
    - All VCFs merged with `bcftools merge -merge all`
    - plink filtering separately for SNPs and SVs
    - LMM test with gemma
    - Also performed "traditional" GWAS with a bwa-GATK pipeline
- [Li et al. Genome Research 2023](https://pmc.ncbi.nlm.nih.gov/articles/PMC10078295/)
    - Pacbio, hifiasm, 
    - SV calling: Sniffles, pbsv, cuteSV, minigraph (from amb)
    - pangenome: minigraph
    - SV genotyping from short reads with paragraph
    - 
    
    
| Study                     | Pangenome                      | SV calling | Association test      | Notes                             |
|---------------------------|--------------------------------|------------|-----------------------|-----------------------------------|
| CochetelGenomeBiology2023 | pggb-de-construct              | vg call    | gemma LMM             |                                   |
| LiGenomeResearch2023      | minigraph                      | paragraph  | gemma LMM             | GWAS on nearby SNPs from SNP chip |
| MiliaBiorxiv2024          | pggb                           | NA         | path Jaccard          | WGS just to confirm               |
| YanNatGen2023             | vg construct                   | vg call    | gemma LMM             | SV merging strategy?              |
| WangBiorxiv2023           | Psvcp/PPsPCP (~linear)         | coverage   | Fisher test           |                                   |
| WangGenomeBiology2023     | PSVCP (~linear)                | coverage   | GAPIT LMM             |                                   |
| VaughnNatComm2022         | ProgressiveMauve               | PanPipes   | GAPIT                 | SV merging strategy?              |
| KangNatComm2023           | minigraph                      | vg         | GCTA LMM (w. kinship) | SV merging strategy?              |
| YildizTheApplGen2025      | vg                             | vg         | matrixEQTL            | SV merged with Jasmine            |
| TaoBiorxiv2025            | vg                             | vg call    |                       |                                   |
| VillaniIScience2025       | pggb                           | vg call    |                       | no GWAS, only PheWAS on 12 SNPs   |
| YaonNatGen2025            | minigraph-cactus / vg constuct | vg call    | rMVP GLM              |                                   |
| GuoNatGen2025             | vg construct                   | vg call    | MatrixEQTL            | SV merged with SURVIVOR           |
| ZhuMolPlant2025           | vg construct                   | vg call    | gemma LMM             | removed repeats, SURIVOR merging  |
| YuMolHortic2025           | vg construct                   |            | no GWAS               | merged PanPop                     |
| ChenGenBiol2025           | minigraph-cactus               | pangenie   | lme4/TASSEL5          |                                   |
| ZhouNature2022            | vg construct                   | vg call    | GCTA LMM              | SV merged with SURVIVOR           |
| ChengNature2025           | minigraph-cactus & pggb        |            |                       |                                   |


- Pangenomic analysis identifies structural variation associated with heat tolerance in pearl millet 
- KangNatComm2023 https://pmc.ncbi.nlm.nih.gov/articles/PMC10558531/ 

## Tools

- gafpack? Used in MiliaBiorxiv2024
