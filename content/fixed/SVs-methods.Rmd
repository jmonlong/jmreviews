...
Using this method, copy number changes at a level of **5–10 kilobases** of DNA sequences can be detected.
...
@Li2021 developped [PanSVR](/2022/07/12/litreviews-svdetection) as a SV caller guided by known SVs. 
It can "genotype" divergent alleles but only around know SV sites.
It uses a pangenomic seeding approach to cluster the reads and assemble them.
...
The gnomAD pipeline was used in a few studies, including the one exploring the transcriptomic effects of rare SVs in brain [@CommonMindConsortium2020].
It consists of: Delly, Lumpy, Manta, Wham, MELT, CNVnator, GenomeSTRiP, cn.MOPS.

---

The Hall lab used [svtools](https://github.com/hall-lab/svtools) (discovery with LUMPY, CNVnator, svtyper; then genotyping) to perform a GWAS on Finns.

---

[BioGraph](https://github.com/spiralgenetics/biograph) was developed by Spiral Genetics and described in a [white paper on bioRxiv](https://www.biorxiv.org/content/10.1101/2020.04.24.060202v1) (startup acquired since then).
The idea is to build a graph of the sequencing reads, once, and then query/call variants efficiently.
With the BioGraph format, the pre-processed reads are not tied to a reference version anymore. 
Under the hood, it seems to be a BWT-like structure that more or less replicates what you can do with a de bruijn graph.
Reads information are indexed in a separate file.
To discover variants, they slide a window on the reference sequence and assemble local contigs from the read graph when they detect a difference.
The contigs are extended until they rejoin the reference path or up to 100Kb of sequence is assembled.
A "coverage" computation can also be performed from variants defined in a VCF.
Finally, a random forest model was trained to genotype variants based on this coverage information.
They showed that it performs better than Manta or Parliament2 for SV discovery.
I'm not sure how it deals with repeats, sequencing errors.
...
[Viola](/2022/04/30/litreviews-svsmethods)[@Sugita2022] merges SVs based on the breakpoints positions, making sure the strand are concordant and SVs overlap on at least 1bp.
It doesn't seem to explicitly ensure that the merged SVs are of the same type. 

[MARS](/2022/04/30/litreviews-svsmethods) [@Zhang2021] calls SVs from diploid assemblies and refine their breakpoints across haplotypes of multiple samples using multiple sequence alignment.

[TT-MARS](/2022/04/30/litreviews-svsmethods) [@Yang2021] also compare the SV calls to an haplotype-resolved assembly but focusing on evaluating the quality of SV calls, one sample at a time.
No breakpoint refinement.
Still a more modern way of evaluating SVs using the new high-quality haplotype-resolved assemblies from HGSVC/HPRC etc.
...
[Viola](/2022/04/30/litreviews-svsmethods)[@Sugita2022] looks for mutation signatures in cancer samples from SV annotations using non-negative matrix factorization.



# Long-read sequencing

pbmm2 + pbsv was used in @Hiatt2020.
They also made *de novo* assemblies with hifiasm and "called" SVs from the minimap2 alignments.

# Single-cell sequencing

## From scRNA-seq

[InferCNV](https://github.com/broadinstitute/inferCNV/wiki) compares expression of tumor cells across the genome to that of normal cells.
The algorithm is as follow:

1. genes expressed only in a few cells are removed
2. depth normalization per cell, adjusting the total to match the median total.
3. log(x+1) transformation
4. for each gene, subtract the average across normal cells
5. winsorize (e.g. force to be in [-3,3] range)
6. smoothing using a running average weighted using a pyramidal scheme (default 101 window).
7. center cell expression around 0
8. for each gene, subtract the average across normal cells (again)
9. revert log-transformation

The CNVs are called at the cell level using HMM approaches.
Either a 3 states version (loss, neutral, gain) or a more granular 6 states one.
It's interesting how they use synthetic data (from the normal cells) as a baseline, to calibrate the HMM results a posteriori.
Finally, the can annotate the CNV calls using a Bayesian Network (and MCMC), and remove calls with high posterior probability of being normal.

Additionally, the modified expression profiles can be further de-noised, for example to clean up the visualization.
One approach forced to 0 values matching the average + 3 SD of the normal cells.
The values can be further cleaned up using median filtering, i.e. a rolling median approach.

It looks like the subcluster identification is performed using hierarchical clustering.
They also mention that other part of the analysis can benefit from subcluster partition, e.g. running the HMM per cluster.
I thought the CNV calling was done at the cell level, I might be missing something.

---

[HoneyBadger](https://github.com/JEFworks/HoneyBADGER) **+++**

# Complex loci

SMN1/2 are two genes located in a particularly variable region with a lot of segmental duplications.
While the genes are ~28 kbp long, they are within paralogous regions of ~205 kbp, 99.8% similar and separated by ~670 kbp.
There seem to be only [one functional base](https://www.ncbi.nlm.nih.gov/clinvar/variation/586628/) that differ between SMN1 and SMN2.
SMN1 is the functional copy but SMN2 can still produce a small quantity of transcript so the SMN2 copy number can be a modifier factor of spinal muscular distrophy's severity.
Individuals can carry 0, 1, or 2 copies of SMN1.
Detecting silent carrier (carrying 2 + 0 copies of SMN1) is also important.

[Paraphase](https://github.com/PacificBiosciences/paraphase) realigns long PacBio reads to one unique copy and phase them into "haplotypes" (phased variants in one copy of a gene) [@chenComprehensiveSMN1SMN22023].
The original publication focused on SMN1/2 and more specifically of the 44 kbp region around the genes.
Reads are phased into haplotypes based on the variant sites and label them as SMN1/2 based on the c.840 site.
SMN1/2 copy numbers are estimated as the number of unique of haplotypes, although depth analysis is also performed when only one copy is detected.
Phasing across "alleles" was done using parental information, either from long-reads or from short reads by focusing on haplogroup-informative sites.
...
@Noyvert2023 built an imputation panel from 888 samples from the 1000GP and imputed SVs in the UK Biobank.
They used ONT sequencing (R9) with ~15x coverage of ~6.2 kbp long reads.
SVs were called with Sniffles2 in joint calling mode.
On average, individuals carried 16K SVs.
They integrated the SV catalog with the 1000GP release 3 (45M) for those same individuals.
Variants were phased with Beagle5.
Beagle5 was also used for the imputation using input SNPs that were phased with SHAPEIT4.
The imputation accuracy was evaluated using leave-one-out comparison.
Two metrics were computed: per-variant non-reference genotype concordance, imputation quality $r^{2}_{imp}$.
They also evaluated the performance per individuals by computing a per-individual non-reference genotype concordance.
The imputed SVs in UKBB resulted in 3,858 SV association being found.
Only 55 of those had a lower p-value than SNVs around (if I read that correctly).
They describe examples where SVs helped prioritize the gene candidate.
